...
int main() {
    ...
    const size_t N = 1 << 20;
    const size_t VEC = 4;                          // 向量宽度：float4
    const size_t ELEMS = N / VEC;                  // 向量元素个数 = 256K
    size_t size_bytes = ELEMS * sizeof(float4);

    std::vector<float4> h_a(ELEMS, make_float4(1, 1, 1, 1));
    std::vector<float4> h_b(ELEMS, make_float4(2, 2, 2, 2));
    std::vector<float4> h_c(ELEMS, make_float4(0, 0, 0, 0));
    ...
    bool verified = cudabench::verify_all(h_c, [](float4 v) {
        return v.x == 3.0f && v.y == 3.0f && v.z == 3.0f && v.w == 3.0f;
    });
    ...
    report.data_type     = "float4";
    report.element_size  = sizeof(float4);
