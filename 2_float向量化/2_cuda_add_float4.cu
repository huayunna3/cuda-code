...
// float4：16 字节、16 字节对齐，make_float4 创建
template <typename T>
...
int main() {
    ...
    const size_t SIZE = 1 << 20;                  // 1M 个 float4 元素（每个 16 字节，数据量 16MB）
    size_t size_bytes = SIZE * sizeof(float4);

    std::vector<float4> h_a(SIZE, make_float4(1, 1, 1, 1));
    std::vector<float4> h_b(SIZE, make_float4(2, 2, 2, 2));
    std::vector<float4> h_c(SIZE, make_float4(0, 0, 0, 0));
    ...
    // 验证结果（注意组件访问方式 .x .y .z .w）
    std::cout << "计算结果验证 (a[0]+b[0]): (" << h_c[0].x << ", " << h_c[0].y << ", " << h_c[0].z
              << ", " << h_c[0].w << ") (预期: (3, 3, 3, 3))" << std::endl;
    std::cout << "计算结果验证 (a[last]+b[last]): (" << h_c[SIZE-1].x << ", " << h_c[SIZE-1].y
              << ", " << h_c[SIZE-1].z << ", " << h_c[SIZE-1].w << ") (预期: (3, 3, 3, 3))"
              << std::endl;
