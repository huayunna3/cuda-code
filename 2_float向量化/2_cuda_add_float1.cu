...
// float1 即标量 float：4 字节、4 字节对齐，无向量化包装
template <typename T>
...
int main() {
    ...
    const size_t N = 1 << 20;
    const size_t VEC = 1;                          // 向量宽度：float1
    const size_t ELEMS = N / VEC;                  // 向量元素个数
    size_t size_bytes = ELEMS * sizeof(float);

    std::vector<float> h_a(ELEMS, 1);
    std::vector<float> h_b(ELEMS, 2);
    std::vector<float> h_c(ELEMS, 0);
